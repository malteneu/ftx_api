...
class FtxClient:
    ...
    def get_borrowing_rates(self) -> List[dict]:
        return self._get('spot_margin/borrowing_rates')

    def get_borrow_summary(self) -> List[dict]:
    ...
    def close_position(self, market: str):
        ...
        current_position = self.get_market_position(market)

        if current_position:
            if str(current_position['side']) == 'buy':
                self.place_order(market, 'sell', 0, current_position['openSize'], 'market', True)
                return True
            elif str(current_position['side']) == 'sell':
                self.place_order(market, 'buy', 0, current_position['openSize'], 'market', True)
                return True
        return False

    def get_market_position(self, market: str):
        positions = self.get_positions()

        for i in range(0, len(positions)):
            if str(positions[i]['entryPrice']) != 'None' and str(positions[i]['future']) == market:
                return positions[i]
                break
        return
    ...
    def get_total_balance(self):
        ...
        for i in range(0, len(balance)):
            total += balance[i]['usdValue']
        return total

    def open_position_with_leverage(self, market: str, side: str, leverage: int, price: float, type: str = 'limit',
                                    reduce_only: bool = False, ioc: bool = False, post_only: bool = False,
                                    client_id: str = None):
        balance = self.get_total_balance()

        market_price = f.get_historical_prices(market, '86400', str(time.time() - 86400), str(time.time()), '1')
        market_price = market_price['close'][len(market_price['close']) - 1]

        size = balance / market_price * leverage

        return self.place_order(market, side, price, size, type, reduce_only, ioc, post_only, client_id)
